...
class AnchorlessImages(tf.keras.callbacks.Callback):
    ...
    def _heatmap_overlay(self, heatmap_pred, heatmap_true, nm, Hoc, lens, dims):
        """Create heatmap visualization overlay with custom color scheme (max aggregation)."""

        dims_i32 = tf.cast(dims, tf.int32)
        H, W = dims_i32[0], dims_i32[1]
        ...
        on_idx = tf.squeeze(tf.where(on_screen), axis=1)
        px_filtered   = tf.gather(px, on_idx)
        pred_filtered = tf.gather(tf.squeeze(heatmap_pred, axis=-1), on_idx)
        true_filtered = tf.gather(tf.squeeze(heatmap_true, axis=-1), on_idx)
        ...
        valid_mask = tf.reduce_all(
            tf.logical_and(px_expanded >= 0, px_expanded < tf.expand_dims(dims, 0)),
            axis=-1
        )
        valid_idx = tf.squeeze(tf.where(valid_mask), axis=1)
        px_valid  = tf.gather(px_expanded, valid_idx)

        # Replicate values for each 3x3 position
        pred_expanded = tf.repeat(pred_filtered, dot_size * dot_size)
        true_expanded = tf.repeat(true_filtered, dot_size * dot_size)
        pred_valid    = tf.gather(pred_expanded, valid_idx)
        true_valid    = tf.gather(true_expanded, valid_idx)

        if tf.size(px_valid) == 0:
            return tf.zeros(tf.concat([dims_i32, [3]], axis=0), dtype=tf.float32)

        # ===== MAX aggregation instead of additive scatter =====
        lin_idx = px_valid[:, 0] * W + px_valid[:, 1]  # [M] linear indices in [0, H*W)

        # Per-pixel max for GT and prediction
        gt_flat   = tf.math.unsorted_segment_max(true_valid, lin_idx, H * W)
        pred_flat = tf.math.unsorted_segment_max(pred_valid, lin_idx, H * W)

        # Empty pixels become -inf; clamp to 0
        gt_flat   = tf.maximum(gt_flat, 0.0)
        pred_flat = tf.maximum(pred_flat, 0.0)

        gt_map   = tf.reshape(gt_flat,   [H, W])
        pred_map = tf.reshape(pred_flat, [H, W])

        # ===== Build overlays (same colour logic as your original) =====
        # Ground truth grayscale
        gt_overlay = tf.stack([gt_map, gt_map, gt_map], axis=-1)  # [H, W, 3]

        # Prediction: red (presence) to yellow (high value)
        pred_red   = tf.where(pred_map > 0.0, 1.0, 0.0)
        pred_green = pred_map
        pred_blue  = tf.zeros_like(pred_map)
        pred_overlay = tf.stack([pred_red, pred_green, pred_blue], axis=-1)

        # Prediction overrides ground truth where present
        pred_mask = tf.reduce_max(pred_overlay, axis=-1, keepdims=True)  # [H, W, 1]
        overlay = gt_overlay * (1.0 - tf.cast(pred_mask > 0.0, tf.float32)) + pred_overlay
